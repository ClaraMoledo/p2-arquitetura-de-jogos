"""Ponto de entrada do players-service (FastAPI)."""
...
def _init_db_with_retry() -> None:
    ...
    while True:
        try:
            ...
            seed()
            print("[players-service] schema de jogadores pronto.")
            return
            ...
            print(f"[players-service] banco indisponível (tentativa {attempt}): {exc}. Retry em {wait}s.")
            time.sleep(wait)
            attempt += 1
...
def create_app() -> FastAPI:
    app = FastAPI(title="Ludoteca — Jogadores", lifespan=lifespan)
    register_error_handlers(app)
    app.include_router(router)

    @app.get("/health")
    def health() -> dict:
        return {"service": "players-service", "status": "ok", "database": _database_status()}
